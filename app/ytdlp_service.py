...
class YtDlpService:
    ...
    def list_formats(self, url: str, max_items: int = 8) -> Tuple[str, List[FormatItem], FormatItem]:
        ...
        for f in info.get("formats", []):
            ...
            formats.append(FormatItem(format_id=fid, label=label, ext="mp4", height=height, filesize=fs))

        formats.sort(key=lambda x: (x.height or 0), reverse=True)
        formats = formats[:max_items]

        audio = FormatItem(format_id="bestaudio/best", label="Только аудио (best)", ext="audio", height=None, filesize=None)
    ...
    def get_direct_url(self, page_url: str, format_id: str) -> Tuple[str, Dict[str, str]]:
        ...
        # В зависимости от источника yt-dlp может вернуть либо 'url', либо requested_formats
