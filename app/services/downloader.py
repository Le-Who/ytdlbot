...
from app.core.config import TEMP_DIR, BASE_URL
...
class MediaSender:
                                    ...
                                    if match:
                                        try:
                                            ...
                                            await progress_callback(
                                                f"{icon} Скачиваю: {render_progressbar(percent)}{details}",
                                                kb_cancel,
                                            )
                                            last_update = now
    ...
    async def send_file(
        ...
        """
        Sends a file to Telegram with automatic retry on transient errors.
        Handles RetryAfter (Telegram rate limit) and NetworkError with exponential backoff.
        """
        last_error = None
        ...
        for attempt in range(1, MAX_SEND_RETRIES + 1):
            try:
                ...
                logger.warning(
                    f"[SEND] RetryAfter {e.retry_after}s (attempt {attempt}/{MAX_SEND_RETRIES})"
                )
                last_error = e
                ...
                logger.warning(
                    f"[SEND] {type(e).__name__} (attempt {attempt}/{MAX_SEND_RETRIES}), retry in {delay}s"
                )
                last_error = e
                if attempt < MAX_SEND_RETRIES:
                ...
                logger.error(f"[SEND] Unrecoverable error: {e}", exc_info=True)
                return False

        logger.error(f"[SEND] All {MAX_SEND_RETRIES} attempts failed: {last_error}")
        return False
