...
class TestGalleryDlService(unittest.TestCase):
    ...
    def test_collect_files_invalid_json(self):
        """_collect_files should ignore invalid/corrupted JSON metadata."""
        with tempfile.TemporaryDirectory() as tmpdir:
            for name in ["001.jpg"]:
                with open(os.path.join(tmpdir, name), "w") as f:
                    f.write("fake image")
    ...
    def test_collect_files_title_fallback(self):
        """_collect_files should fallback to 'title' if 'description' is absent."""
        with tempfile.TemporaryDirectory() as tmpdir:
            for name in ["001.jpg"]:
                with open(os.path.join(tmpdir, name), "w") as f:
                    f.write("fake image")

            meta = {"description": "", "title": "Fallback title"}
            with open(os.path.join(tmpdir, "meta.json"), "w") as f:
                json.dump(meta, f)
