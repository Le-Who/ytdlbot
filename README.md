...
- **Fast-Path Media Pipeline (Zero-CPU/Zero-Disk)**: Five complementary optimizations minimize server load and latency:
  ...
  - **OPT-7 Redis Subprocess Throttling**: Eliminated synchronous network bottlenecks during high-throughput downloads by decoupling Redis cancellation cache checks from the fast-firing `yt-dlp` stdout buffer.
